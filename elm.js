...
Elm.ElmerPress.make = function (_elm) {
   ...
   var switchTurn = function (model) {
      return function () {
         ...
         var markIfLettersLocked = function (board) {
            return A2($List.map,
            markIfLetterLocked(board),
            board);
         };
         ...
         var markLettersColors = function (board) {
            return A2($List.map,
            markLetterColor,
            board);
         };
         var newBoard = function ($) {
            return markIfLettersLocked(markLettersColors($));
         }(model.board);
         return _U.replace([["board"
                            ,newBoard]
